...
public class OrderSearchSpecification implements Specification<OrderEntity> {

    private OrderSearchDTO searchDTO;

    public OrderSearchSpecification(OrderSearchDTO searchDTO) {
    ...
    public Predicate toPredicate(Root<OrderEntity> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
        ...
        if (StringUtils.isNotBlank(searchDTO.getSearchText())) {
            ...
            predicates.add(cb.like(root.get("user").get("userId"), likeText));
            predicates.add(cb.like(root.get("user").get("userName"), likeText));
