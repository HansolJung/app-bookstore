...
public class AdminUserController {
    ...
    public ModelAndView getUpdateForm(@PathVariable(name = "userId") String userId) {
        ModelAndView view = new ModelAndView();
    ...
    public ModelAndView getOrderListView(@PageableDefault(page = 0, size = 10, 
        ...
        try {
            ...
            log.info("### 주문 내역 리스트 출력 ###", resultMap);
        } catch (Exception e) {
            log.error("######## 주문 내역 리스트 출력 에러 ########");
            e.printStackTrace();
        }
    ...
    public ModelAndView getOrderDetailView(@PathVariable(name = "orderId") int orderId) {
        ...
        ModelAndView view = new ModelAndView();
        OrderDTO.Detail dto = null;

        try {
            dto = orderService.getOrder(orderId);
            log.info("### 주문 상세 출력 ###", dto);
        } catch (Exception e) {
            log.error("######## 주문 상세 출력 에러 ########");
            e.printStackTrace();
        }
